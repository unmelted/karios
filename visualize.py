...
class Visualize() :
    ...
    def generate_heatmap(cls, job_id, start_frame, end_frame) :
        ...
        grids = [[init_val for _ in range(defn.horizontal_grid)] for _ in range(defn.vertical_grid)] 
        ...
        for row in data :
            ...
            grids[int(row[1]/unit_v)][int(row[2]/unit_h)] += 1
    ...
    def generate_3d_data(cls, job_id, select_frame) :
        result = 0
        data = DbManager.get_players_3d_1frame(job_id, select_frame)     
        return result, data

    @classmethod
    def generate_2d_data(cls, job_id, select_frame) :
        result = 0
        data = DbManager.get_players_2d_1frame(job_id, select_frame)
